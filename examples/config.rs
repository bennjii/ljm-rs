...
fn read() {
    ...
    let read_value =
        LJMLibrary::get_config("LJM_STREAM_SCANS_RETURN".to_string()).expect("Expected Value");

    println!("Got configuration value: {}", read_value);

    LJMLibrary::set_config("LJM_STREAM_SCANS_RETURN".to_string(), 2).expect("Expected Value");
